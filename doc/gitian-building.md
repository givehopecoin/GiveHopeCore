...
*Setup instructions for a gitian build of GiveHope using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the GiveHope
...
- [Building GiveHope](#building-givehope)
...
To build GiveHope (for Linux, OSX and Windows) just follow the steps under 'perform
...
For example:
```bash
URL=https://github.com/givehopecoin/GiveHopeCore.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit givehope=${COMMIT} --url givehope=${URL} ../givehope/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit givehope=${COMMIT} --url givehope=${URL} ../givehope/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit givehope=${COMMIT} --url givehope=${URL} ../givehope/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/givehope-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/givehope-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/givehope-build.assert
```
...
[givehope/gitian.sigs](https://github.com/givehopecoin/GiveHopeCore/gitian.sigs/) repository, or if that's not possible to create a pull
request.
