...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::HOPC)
    {
    }
                                              ...
                                              filter(0)
{
    ...
    QSettings settings;
    int _NUM_ITEMS = settings.value("nDisplayObjects").toInt();
    ...
    ui->listTransactions->setMinimumHeight(_NUM_ITEMS * (DECORATION_SIZE + 2));
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("HOPC")
    updateDisplayUnit();
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeHOPCAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeHOPCAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeHOPCAmount = strAnonymizeHOPCAmount.remove(strAnonymizeHOPCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHOPCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeHOPCAmount * COIN) nMaxToAnonymize = nAnonymizeHOPCAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeHOPCAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeHOPCAmount));
        strAnonymizeHOPCAmount = strAnonymizeHOPCAmount.remove(strAnonymizeHOPCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHOPCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
