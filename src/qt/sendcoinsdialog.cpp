...
void SendCoinsDialog::send(QList<SendCoinsRecipient> recipients, QString strFee, QStringList formatted)
{
    ...
    QSettings settings;
    int MAX_DISPLAY_OBJECTS = settings.value("nDisplayObject").toInt();

    for (int i = 0; i < formatted.size(); i++) {
//        if (i >= MAX_SEND_POPUP_ENTRIES) {
        if (i >= MAX_DISPLAY_OBJECTS) {
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid HOPC address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
