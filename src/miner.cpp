...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GiveHopeMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("GiveHopeMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("GiveHopeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("givehope-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("GiveHopeMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running GiveHopeMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadGiveHopeMiner() exception");
    } catch (...) {
        LogPrintf("ThreadGiveHopeMiner() exception");
    }

    LogPrintf("ThreadGiveHopeMiner exiting\n");
}
