...
#if defined(NDEBUG)
#error "GiveHope cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "GiveHope Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0)                                  // BLOCK: 0           : 450,000 Premine   TOTAL    450,000 (2.5%)
        nSubsidy = 450000 * COIN;
    else if (nHeight < Params().LAST_POW_BLOCK())      // < BLOCK: 501       :       0           TOTAL    450,000
        nSubsidy = 0 * COIN;
    else if (nHeight >= 500 && nHeight <= 10000)
        nSubsidy = 0.3 * COIN;
    else if (nHeight >= 10000 && nHeight <= 20000)
         nSubsidy = 2.5 * COIN;
    else if (nHeight >= 20000 && nHeight <= 45000)
        nSubsidy = 25 * COIN;
...
{
    RenameThread("givehope-scriptch");
